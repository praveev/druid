...
COMPONENT_NAME=digits_druid_broker
APPLICATION_MAIN_CLASS=com.yahoo.digits.druid.daemon.BrokerDaemon

#PROFILING="-agentlib:hprof=cpu=samples,depth=8,interval=5,thread=y,doe=y,file=/tmp/smq_broker.hprof -verbose:gc"
...
#digits jar
CLASSPATH=${CLASSPATH}:$YINST_ROOT/conf/${YINST_VAR_basePkg}/broker
...
JVM_ARGS="-XX:MaxDirectMemorySize=${edigits_druid__historicalDirectMem} -Xmx${edigits_druid__brokerMaxMem} ${edigits_druid__brokerJavaOpts} -Duser.timezone=UTC -Dfile.encoding=UTF-8 -Djava.io.tmpdir=$YINST_ROOT/tmp/$YINST_VAR_basePkg ${JVM_ARGS}"
